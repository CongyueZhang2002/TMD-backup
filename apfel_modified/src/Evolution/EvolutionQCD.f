...
*
      ...
      double precision f3ph(-6:6,0:nint_max)
...
*
*     Save a copy of the nf=3 pdfs for using with the NPVFNS: 
*
      do alpha=0,nin(igrid)
         do ifl=-6,6
            if(abs(ifl).gt.3) then
             f3ph(ifl,alpha) = 0d0
            else
             f3ph(ifl,alpha) = f0ph(ifl,alpha)
            endif
         enddo
      enddo
*
*     Rotate initial PDFs into the evolution basis, whilst checking if NPVF is on
*

      if (Evs.eq."VF".and.NPVF) then
         call PDFphys2evQCD(f3ph,f0ev)
      else
...
*
*     Find initial and final number of flavours for NPVFNS
*
            if(muF2.gt.m2th(6))then
               nff = 6
            elseif(muF2.gt.m2th(5))then
            ...
            if(nff.gt.nfmax) nff = nfmax
*
            if(muF20.gt.m2th(4))then
               write(6,*) "With NPVF option initial scale",
     1           " cannot go beyond charm mass for now!"
               stop
            else
               nfi = 3
            endif
...
*     If initial and final energies are equal, return immediately the intial conditions
           ...
           elseif(muF2.gt.muF20)then
              sgn = 1
           elseif(muF2.lt.muF20)then
               write(6,*) "With NPVF backward evolution is not",
     1           " possible for now!"
               stop
           endif
*
...
*
              ...
              enddo
               ...
               if(jnf.eq.4)then
                  do alpha=0,nin(igrid)
                     V0(alpha)  = V0(alpha) + (f0ph(4,alpha)-
     1                                           f0ph(-4,alpha))
                     V150(alpha) = V0(alpha) - 4d0*(f0ph(4,alpha)-
     1                                           f0ph(-4,alpha))
                     Sg0(1,alpha) = Sg0(1,alpha)+ (f0ph(4,alpha)+
     1                                           f0ph(-4,alpha))
                     T150(alpha) = Sg0(1,alpha) - 4d0*(f0ph(4,alpha)+
     1                                           f0ph(-4,alpha))
                  enddo
               endif
               if(jnf.eq.5)then
                  do alpha=0,nin(igrid)
                     V0(alpha)  = V0(alpha) + (f0ph(5,alpha)-
     1                                           f0ph(-5,alpha))
                     V240(alpha) = V0(alpha) - 5d0*(f0ph(5,alpha)-
     1                                           f0ph(-5,alpha))
                     Sg0(1,alpha) = Sg0(1,alpha)+ (f0ph(5,alpha)+
     1                                           f0ph(-5,alpha))
                     T240(alpha) = Sg0(1,alpha) - 5d0*(f0ph(5,alpha)+
     1                                           f0ph(-5,alpha))
                  enddo
               endif
