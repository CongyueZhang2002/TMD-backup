...
C-----1: u   2:  d    3: s   4: ub   5: db   6: sb
     ...
     >     bmax,ktw,ptw,Qini,kappa2
      ...
      call EPPS16_proton(x,Q,AA,ZZ,U,D,UB,DB,SS,SB,GL,nloops)
      endif
      ...
      if (AA.eq.4) then
      call evolvePDFM(2,x,Q,pdf) ! He
      elseif (AA.eq.20) then
      call evolvePDFM(3,x,Q,pdf) ! Ne
      elseif (AA.eq.84) then
      call evolvePDFM(4,x,Q,pdf) ! Kr
      elseif (AA.eq.131) then
      call evolvePDFM(5,x,Q,pdf) ! Xe
      elseif (AA.eq.9) then
      call evolvePDFM(10,x,Q,pdf) ! Be
      elseif (AA.eq.56) then
      call evolvePDFM(11,x,Q,pdf) ! Fe
      elseif (AA.eq.184) then
      call evolvePDFM(12,x,Q,pdf) ! W
      elseif (AA.eq.197) then
      call evolvePDFM(20,x,Q,pdf) ! Au
      elseif (AA.eq.1) then
      call evolvePDFM(21,x,Q,pdf) ! p
      endif
