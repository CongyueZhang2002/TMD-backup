...
c--------------------------------------------------------------
      ...
      real*8 Q2,wid, widuu, Qaux
      integer IT,Ixfy,nloops,hop,nll,pre,ZZ
...
*-----TMD PDF2
      ...
      elseif(IT.eq.9) then  ! Be target
          ZZ = 4
      call PDF_A_LHAPDF(xb,Q0,IT,ZZ,U2,D2,UB2,DB2,SS2,SB2,GL2,nloops)
        !                      (A,Z)
...
*---- with Modifications to account for Nuclear Medium Effects:
      if (IT.eq.3) then
      wid = ktw + aN*(2.**(1d0/3d0)-1d0) !*(xb**g2a)*((1-xb)**a2)
      widuu = kappa2*dlog(Q/Qini)
c     >        + gP*(2.**(1d0/3d0)-1d0)*((Qaux/Qini)**aP)
      else
      wid = ktw + aN*(IT**(1d0/3d0)-1d0) !*(xb**g2a)*((1-xb)**a2)
      widuu = kappa2*dlog(Q/Qini)
C   >        + gP*(IT**(1d0/3d0)-1d0)*((Qaux/Qini)**aP)
      endif

      FNPN=dexp(-wid/4d0*b*b-widuu/2d0*dlog(b/bstar))
