      ...
      subroutine PDF_A_LHAPDF(x,Q,A,ZZ,U,D,UB,DB,SS,SB,GL,nloops)
      implicit none
      real*8 x,Q,U,D,UB,DB,SS,SB,GL,pdf(-6:6)
      ...
      if (nloops.eq.2) then
          call PDF_LHAPDF_NLO(x,Q,A,ZZ,Un,Dn,UBn,DBn,SSn,SBn,GLn)
          U =U +Un
          D =D +Dn
          SS=SS+SSn
          SB=SB+SBn
...
c----------------------------------------------------------------
c     PDF in nucleus ( EPPS21 )
c----------------------------------------------------------------
      subroutine PDF_A_EPPS21(x,Q,AA,ZZ,U,D,UB,DB,SS,SB,GL,nloops)
      implicit none
      real*8 x,Q
      ...
      if (nloops.eq.2) then
          call PDF_EPPS21_NLO(x,Q,AA,ZZ,Un,Dn,UBn,DBn,SSn,SBn,GLn)
          U =U +Un
          D =D +Dn
          SS=SS+SSn
          SB=SB+SBn
...
c-----------------------------------------------------------------------
      ...
      subroutine PDF_EPPS21_NLO(x,Q,AA,ZZ,UN,DN,UBN,DBN,SSN,SBN,GL)
      implicit none
      real*8 x,Q,cxf
...
c-----------------------------------------------------------------------
      ...
      subroutine PDF_LHAPDF_NLO(x,Q,AA,ZZ,UN,DN,UBN,DBN,SSN,SBN,GL)
      implicit none
      real*8 x,Q,cxf
      real*8 U,D,UB,DB,SS,SB,GL,UN,DN,UBN,DBN,SSN,SBN,GLN
      real*8 cu,cd,cub,cdb,css,csb,cg
      ...
      call conv_qgauss_PDF_LHAPDF(10,x,Q,AA,ZZ,cu,cd,cub,cdb,css,csb,cg)
...
C-------------------------------------------------
     ...
     >     'WARNING: Too high accuracy required for QGAUSS!'
      ...
      Do 100 j=1,8
         d=xr*xd(j)
         call PDF_A_EPPS21(x/(xm+d),Q,A,Z,
...
c     Int_x_i^x_f func(x) dx using gaussian integration (for convolving)
c-----------------------------------------------------------------------
      subroutine conv_qgauss_PDF_LHAPDF(n,x,Q,A,Z,cu,cd,cub,
     >                               cdb,cs,csb,cg)
      implicit none
      real*8 xi,xf,xn,x1,x2,x,Q,cu,cd,cub,cdb,cs,csb,cg
      real*8 cui,cdi,cubi,cdbi,csi,csbi,cgi
      ...
      if(n.le.1) then           ! same as n=1
         x1=x
         x2=1d0
         call conv_gauss_PDF_LHAPDF(x1,x2,x,Q,A,Z,cui,
     >        cdi,cubi,cdbi,csi,csbi,cgi)
         cu =cui
         cd =cdi
         cub=cubi
...
C-------------------------------------------------
      subroutine conv_gauss_PDF_LHAPDF(xi,xf,x,Q,A,Z,
     >           cu,cd,cub,cdb,cs,csb,cg)
      implicit none
      real*8 cu,cd,cub,cdb,cs,csb,cg
      real*8 xi,xf,xm,xr,d,xd(8),w(8),eps,x,Q
     ...
     >     'WARNING: Too high accuracy required for QGAUSS!'
      ...
      Do 100 j=1,8
         d=xr*xd(j)
         call PDF_A_LHAPDF(x/(xm+d),Q,A,Z,
     >                  Up,Dp,UBp,DBp,SSp,SBp,GLp,1)
         call PDF_A_LHAPDF(x/(xm-d),Q,A,Z,
     >                  Um,Dm,UBm,DBm,SSm,SBm,GLm,1)
         gluon=gq(xm+d,Q)*GLp+gq(xm-d,Q)*GLm
