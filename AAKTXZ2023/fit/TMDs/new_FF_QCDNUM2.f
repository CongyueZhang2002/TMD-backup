...
C----- In setting a universal scale Q02 = 1 GeV 
      ...
      call allfxq(iset,zz,QQ2,pdf,0,1) 
      if (IC.eq.1) then
      fu   = pdf(2)/zz
      fub  = pdf(-2)/zz
      fd   = pdf(1)/zz
      ...
      elseif (IC.eq.0) then
      fu  =(pdf(2)/zz+ pdf(-2)/zz)/2d0
      fd  =(pdf(1)/zz+ pdf(-1)/zz)/2d0
      fs  =(pdf(3)/zz+ pdf(-3)/zz)/2d0
      fc  =(pdf(4)/zz+ pdf(-4)/zz)/2d0
      fb  =(pdf(5)/zz+ pdf(-5)/zz)/2d0
...
*
      ...
      REAL*8 gamma, g3f, g3D, p_10, p_11, p_12 !2023
...
C      print*, "nuclear mass: ", A 
     ...
     1              + gi(i) * EBETA(2.d0+ai(i), bi(i)+di(i)+1.d0))
            ...
            ai(i) = ai(i) + gq1 *(1d0-A**gq2)
            bi(i) = bi(i) + dq1 *(1d0-A**dq2)
            gi(i) = gi(i) 
            di(i) = di(i)          
      ENDDO
